...
class MultipleFileUploadXblockTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = mock.patch(
            "edx_mfu.mfu.default_storage",
            FileSystemStorage(tmp))
        patcher.start()
        self.addCleanup(patcher.stop)

    def make_one(self, **kw):
        from edx_mfu.mfu import MultipleFileUploadXBlock as cls
    ...
    @mock.patch('edx_mfu.mfu._resource', DummyResource)
    @mock.patch('edx_mfu.mfu.render_template')
    @mock.patch('edx_mfu.mfu.Fragment')
    def test_student_view(self, Fragment, render_template):
        ...
        fragment.add_css.assert_called_once_with(
            DummyResource("static/css/edx_mfu.css"))
        fragment.add_javascript.assert_called_once_with(
            DummyResource("static/js/src/edx_mfu.js"))
        fragment.initialize_js.assert_called_once_with(
            "MultipleFileUploadXBlock")
    ...
    def test_save_mfu(self):
        block = self.make_one()
        block.save_mfu(mock.Mock(body='{}'))
        self.assertEqual(block.display_name, "Staff Graded Assignment")
        self.assertEqual(block.points, 100)
        self.assertEqual(block.weight, None)
        block.save_mfu(mock.Mock(method="POST", body=json.dumps({
